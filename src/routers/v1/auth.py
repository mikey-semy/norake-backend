...
- Вход в систему (login) для всех ролей (admin/buyer)
...
class AuthRouter(BaseRouter):
    ...
    Public Endpoints:
        POST /auth/login - Вход в систему (admin/buyer)
        POST /auth/refresh - Обновление токенов
        POST /auth/logout - Выход из системы
        GET /auth/me - Получение текущего пользователя

    Архитектурные особенности:
        - Поддержка ролей: admin (полный доступ) и buyer (покупатель)
    ...
    def configure(self):
        ...
        @self.router.post(
            ...
            Аутентифицирует пользователя (admin или buyer) по email и паролю,
            возвращает JWT токены для доступа к API.

            ### Поддерживаемые роли:
            * **admin** - Администратор системы (полный доступ)
            * **buyer** - Покупатель (представитель компании)
        ...
        @self.router.get(
            ...
            * **role**: Роль пользователя (admin/buyer)
            * **company**: Данные компании (для buyer)
