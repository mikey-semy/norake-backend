...
class AuthRouter(BaseRouter):
    ...
    def configure(self):
        ...
        @self.router.get(
            ...
            **🔒 Защищенный эндпоинт** - требуется валидный access токен.

            Возвращает информацию об аутентифицированном пользователе
            на основе токена доступа. Используется фронтендом для:

            * **Инициализации приложения** - проверка залогинен ли пользователь при загрузке SPA
            * **Восстановления сессии** - получение данных пользователя после перезагрузки страницы
            * **Проверки прав доступа** - определение роли для отображения UI элементов
            * **Валидации токена** - проверка что токен еще действителен

            ### Требуется:
            * Валидный access токен в заголовке Authorization или в cookies
            * Токен НЕ должен быть в blacklist (после logout)
            ...
            * **is_active**: Статус активности аккаунта
            ...
            // Сохраняем данные пользователя в store (Redux/Pinia/Zustand)
            ```
            """,
            responses={
                ...
                419: {"description": "Токен просрочен"},
                422: {"description": "Токен в blacklist или пользователь не найден"},
            },
        )
        async def get_current_user(
            ...
            🔒 **Защищенный эндпоинт**: Автоматическая валидация токена через CurrentUserDep.

            Args:
                current_user: Текущий пользователь из dependency injection (автоматическая проверка токена).
            ...
            Raises:
                TokenMissingError: Если токен доступа отсутствует (обрабатывается глобально).
                TokenExpiredError: Токен истек (обрабатывается глобально).
                TokenInvalidError: Токен недействителен или в blacklist (обрабатывается глобально).
                InvalidCredentialsError: Пользователь не найден в системе (обрабатывается глобально).

            Note:
                Этот эндпоинт использует CurrentUserDep, который автоматически:
                - Извлекает токен из заголовка Authorization или cookies
                - Декодирует JWT токен
                - Проверяет токен на валидность и срок действия
                - Проверяет что токен не в blacklist
                - Загружает данные пользователя из базы с eager loading ролей
                - Возвращает готовую схему UserCurrentSchema
            """
            # CurrentUserDep уже вернул валидного пользователя с проверенным токеном
            # Просто оборачиваем в response schema
