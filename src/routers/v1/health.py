...
from src.core.dependencies.health import HealthServiceDep
from src.routers.base import BaseRouter
from src.schemas import HealthCheckDataSchema, HealthCheckResponseSchema
...
class HealthRouter(BaseRouter):
    ...
    def configure(self):
        @self.router.get(
            ...
            Возвращает статусы доступности всех сервисов: приложение, база данных.

            ### Returns:
            * success: успешность запроса
            * message: сообщение о статусе
            * data: словарь статусов сервисов (app, db)
            """,
        )
        async def health_check(
            ...
            return HealthCheckResponseSchema(
                success=True, message="Все сервисы работают", data=data
            )

        @self.router.get(
            path="/live",
            response_model=HealthCheckResponseSchema,
            # summary="Быстрая проверка жизнеспособности",
            description="""
        ...
        async def liveness_check(
            ...
            return HealthCheckResponseSchema(
                success=True, message="Приложение работает", data=data
            )
