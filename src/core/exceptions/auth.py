...
1. BaseAPIException (основной класс для всех API-исключений)
   └── AuthenticationError (базовый класс для ошибок аутентификации)
       ...
       ├── InvalidCurrentPasswordError (неверный текущий пароль)
       ├── WeakPasswordError (слабый пароль)
       ├── InvalidUserDataError (некорректные данные пользователя для токена)
...
class TokenExpiredError(TokenError):
    ...
    def __init__(self, detail: str = "Токен просрочен"):
        """
        ...
        super().__init__(
            ...
            status_code=419,  # Нестандартный код для просроченных токенов (не определен в starlette.status)
        )
...
class InvalidUserDataError(AuthenticationError):
    ...
    Возникает при валидации данных пользователя для создания токена,
    когда отсутствуют обязательные атрибуты или их значения некорректны.

    Attributes:
        detail (str): Описание проблемы с данными пользователя.
    ...
    def __init__(
        self,
        detail: str = "Некорректные данные пользователя",
        extra: Optional[Dict[str, Any]] = None,
    ):
        """
        Инициализирует исключение InvalidUserDataError.

        Args:
            detail (str): Описание проблемы с данными пользователя.
            extra (Dict[str, Any], optional): Дополнительная информация об ошибке.
        """
        super().__init__(
            detail=detail,
