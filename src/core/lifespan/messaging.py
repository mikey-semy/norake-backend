...
from src.core.connections.messaging import RabbitMQClient
from src.core.lifespan.base import (register_shutdown_handler,
...
async def initialize_messaging(app: FastAPI):
    ...
    try:
        ...
        # Try to call declare_exchange if the client exposes it; use getattr to avoid static lint errors
        declare_fn = getattr(client, "declare_exchange", None)
        if callable(declare_fn):
            try:
                logger.info("🔁 Декларация exchange и queue для готовности messaging...")
                await declare_fn(app.state, name=None)
            except Exception:
                logger.debug("ℹ️ declare_exchange helper failed; skipping explicit declare")
        else:
            logger.debug("ℹ️ RabbitMQClient has no declare_exchange helper; skipping explicit declare")
