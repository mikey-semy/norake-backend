            ...
            for attempt in range(self._max_retries):
                try:
                    ...
                    if attempt < self._max_retries - 1:
                        ...
                        self._client = None
                        self.logger.warning(
                            "RabbitMQ недоступен после всех попыток"
                        )

                        # В режиме разработки не выбрасываем исключение
                        if self._debug_mode:
                            self.logger.warning("Приложение продолжит работу без RabbitMQ (DEBUG режим)")
                            ...
                            raise AMQPConnectionError("Не удалось подключиться к RabbitMQ после всех попыток")
...
class RabbitMQContextManager(BaseContextManager[AbstractRobustConnection]):
    ...
    async def connect(self) -> AbstractRobustConnection:
        ...
        """
        try:
            self.logger.debug("Создание подключения к RabbitMQ в контекстном менеджере...")
            self._client = await connect_robust(**self._connection_params)
            self.logger.info("Подключение к RabbitMQ установлено в контекстном менеджере")
            return self._client
        except AMQPConnectionError as e:
            self.logger.error("Ошибка подключения к RabbitMQ в контекстном менеджере: %s", str(e))
            if self._debug_mode:
    ...
    async def close(self) -> None:
        ...
        if self._client:
            try:
                self.logger.debug("Закрытие подключения к RabbitMQ в контекстном менеджере...")
                await self._client.close()
                self.logger.info("Подключение к RabbitMQ закрыто в контекстном менеджере")
            except Exception as e:
                self.logger.error("Ошибка при закрытии подключения к RabbitMQ: %s", str(e))
            finally:
                self._client = None
