...
def format_code():
    ...
    1. black src/ - форматирование Python кода
    2. isort src/ - сортировка импортов
...
def init_minio():
    ...
    Создает bucket 'equiply-documents' если он не существует.
...
def test():
    ...
    if not test_db_connection(env_file_path=env_file_path):
        print("❌ Тестовая инфраструктура не готова! Сначала запусти: uv run infra-test")
        return
...
def start_test_infrastructure():
    ...
    Использует .env.test, отдельные порты и контейнеры.
    Полный цикл: проверка портов, запуск Docker, миграции, вывод адресов.
    """
    print("🚀 Запускаем тестовую инфраструктуру...")
    ...
    if busy_ports:
        ...
        for port_info in busy_ports:
            print(f"   - {port_info}")
        print("💡 Останови процессы на этих портах или измени порты в .env.test")
        return False
    ...
    try:
        ...
        # Получаем порты из .env.test или дефолтные
        ports = {}
        for service, default_port in DEFAULT_PORTS.items():
            if service == 'POSTGRES':
                postgres_port = int(env_vars.get('POSTGRES_PORT', default_port))
                if not is_port_free(postgres_port):
                    print(f"❌ Порт {postgres_port} для PostgreSQL занят!")
                    print(f"💡 Освободи порт {postgres_port} или измени POSTGRES_PORT в .env.test")
                    return False
                ports[service] = postgres_port
            else:
                ports[service] = get_available_port(default_port)
